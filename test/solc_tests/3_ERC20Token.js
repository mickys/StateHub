...
describe("ERC777 - ERC20 Token compatibility", function () {
    ...
    before(async function () {
        ...
        HST = await helpers.utils.deployNewContractInstance(helpers, "TestERC777Token", {
            from: creator,
            arguments: [
    ...
    beforeEach(async function () {
        await restoreFromSnapshot(testKey, false);
    });

    it("creation: in contract settings should match settings", async () => {
        expect(await HST.methods.name().call()).to.equal(setup.settings.token.name);

        expect(await HST.methods.symbol().call()).to.equal(
            setup.settings.token.symbol
        );

        expect(await HST.methods.decimals().call()).to.equal(
            setup.settings.token.decimals.toString()
        );

        expect((await HST.methods.totalSupply().call()).toString()).to.be.equal(
    ...
    it("transfers: should handle zero-transfers normally", async () => {
        assert(
            await HST.methods.transfer(accounts[1], 0).send({ from: creator }),
    ...
    it("approvals: msg.sender approves accounts[1] of 100 & withdraws 20 once.", async () => {
        ...
        HST.methods
            .transferFrom(creator, accounts[2], 20)
            .call({ from: accounts[1] });

        await HST.methods.allowance(creator, accounts[1]).call();
        await HST.methods
            .transferFrom(creator, accounts[2], 20)
            .send({ from: accounts[1] }); // -20
        const allowance01 = await HST.methods
            .allowance(creator, accounts[1])
            .call();
        ...
        assert.strictEqual(
            balance02,
            mintedSupply.sub(new helpers.BN("20")).toString()
        );
    });
    ...
    it("approvals: msg.sender approves accounts[1] of 100 & withdraws 20 twice.", async () => {
        ...
        const allowance012 = await HST.methods
            .allowance(creator, accounts[1])
            .call();
        ...
        await HST.methods
            .transferFrom(creator, accounts[2], 20)
            .send({ from: accounts[1] });
        const allowance013 = await HST.methods
            .allowance(creator, accounts[1])
            .call();
        ...
        assert.strictEqual(
            balance02.toString(),
            mintedSupply.sub(new helpers.BN("40")).toString()
        );
    });
    ...
    it("approvals: msg.sender approves accounts[1] of 100 & withdraws 50 & 60 (2nd tx should fail)", async () => {
        ...
        await HST.methods
            .transferFrom(creator, accounts[2], 50)
            .send({ from: accounts[1] });
        const allowance012 = await HST.methods
            .allowance(creator, accounts[1])
            .call();
        ...
        assert.strictEqual(
            balance0.toString(),
            mintedSupply.sub(new helpers.BN("50")).toString()
        );
    ...
    it("approvals: allow accounts[1] 100 to withdraw from creator. Withdraw 60 and then approve 0 & attempt transfer.", async () => {
        ...
        return helpers.assertInvalidOpcode(async () => {
            await HST.methods
                .transferFrom(creator, accounts[2], 10)
                .send({ from: accounts[1] });
        }, "SafeMath: subtraction overflow");
    });
    ...
    it("allowance: should start with zero", async function () {
        const preApproved = await HST.methods
            .allowance(creator, accounts[1])
            .call();
    ...
    it("approvals: approve max (2^256 - 1)", async () => {
        await HST.methods
            .approve(accounts[1], helpers.MAX_UINT256.toString())
            .send({ from: creator });
        const allowance = await HST.methods.allowance(creator, accounts[1]).call();
